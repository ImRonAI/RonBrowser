/**
 * AgentOrchestrationNode Component
 *
 * Enhanced node component for multi-agent orchestration visualization.
 * Extends StrandsNode pattern with integrated streaming components:
 * - Chain-of-thought visualization
 * - Reasoning display
 * - Tool execution tracking
 * - Image rendering
 * - Purple glow for active streaming agents
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Status Configuration with Active Streaming State
// ─────────────────────────────────────────────────────────────────────────────
...
// Active streaming glow (purple) - overrides status glow
const ACTIVE_STREAMING_GLOW = "shadow-[0_0_32px_-4px_rgba(139,92,246,0.5)] animate-pulse-glow";
...
interface AgentOrchestrationNodeProps {
  agent: AgentConfig;
  status: AgentStatus;
  result?: AgentResult;
...
export const AgentOrchestrationNode = memo(function AgentOrchestrationNode({
  agent,
  status,
  result,
  ...
  canHandoffTo = [],
  isSelected = false,
}: AgentOrchestrationNodeProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const config = STATUS_CONFIG[status];

  // Override glow if actively streaming
  const glowClass = isActivelyStreaming ? ACTIVE_STREAMING_GLOW : config.glow;

  const hasStreamingContent =
    streamingData?.reasoning ||
    (streamingData?.chainOfThought && streamingData.chainOfThought.length > 0) ||
    (streamingData?.tools && streamingData.tools.length > 0) ||
    ...
    >
      ...
      >
        <div className="flex items-start gap-3">
          ...
          >
            {status === "running" || isActivelyStreaming ? (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {isActivelyStreaming && (
                <span className="px-1.5 py-0.5 text-[9px] font-bold bg-violet-500/80 text-white rounded animate-pulse">
                  LIVE
                </span>
              )}
            </div>
            ...
            {agent.modelProvider && (
              <div className="flex items-center gap-1.5 mt-2">
                ...
                <span className="text-[9px] font-medium text-slate-500 uppercase tracking-wide">
                  {agent.modelProvider}
                  {agent.modelId &&
                    ` · ${agent.modelId.split("-").slice(-2).join("-").toUpperCase()}`}
                </span>
              </div>
            )}
          </div>
      ...
      {/* Expanded Details with Streaming Components */}
      <AnimatePresence>
        {isExpanded && (
          <motion.div
                ...
                <div className="space-y-2">
                  ...
                  {streamingData?.reasoning && (
                    <div className="p-2 bg-violet-500/10 border border-violet-500/20 rounded-lg">
                      <span className="text-[9px] font-semibold text-violet-400 uppercase tracking-wider block mb-1">
                        Reasoning
                      </span>
                  ...
                  {streamingData?.chainOfThought &&
                    streamingData.chainOfThought.length > 0 && (
                      <div className="p-2 bg-sky-500/10 border border-sky-500/20 rounded-lg">
                        <span className="text-[9px] font-semibold text-sky-400 uppercase tracking-wider block mb-1">
                  ...
                  {streamingData?.tools && streamingData.tools.length > 0 && (
                    <div className="p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                      <span className="text-[9px] font-semibold text-emerald-400 uppercase tracking-wider block mb-1">
                        Tools ({streamingData.tools.length})
                      </span>
                      <div className="space-y-1">
                        {streamingData.tools.slice(0, 2).map((tool: any, i) => (
                          <div
                            key={i}
                        ...
                        {streamingData.tools.length > 2 && (
                          <span className="text-[9px] text-slate-500">
                            +{streamingData.tools.length - 2} more
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {streamingData?.images && streamingData.images.length > 0 && (
                    <div className="p-2 bg-slate-700/30 border border-slate-600/30 rounded-lg">
                      <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider block mb-1">
                        Images ({streamingData.images.length})
                      </span>
                      {/* Image component would render here */}
...
type GraphNodeComponentProps = NodeProps & {
  data: GraphNodeData;
};

export function GraphNodeComponent({ data }: GraphNodeComponentProps) {
  return (
    <AgentOrchestrationNode
      ...
      isActivelyStreaming={data.isActivelyStreaming}
      streamingData={data.streamingData}
    />
  );
}

type SwarmNodeComponentProps = NodeProps & {
  data: SwarmNodeData;
};

export function SwarmNodeComponent({ data }: SwarmNodeComponentProps) {
  return (
    <AgentOrchestrationNode
      agent={data.agent}
      status={data.status}
...
// WorkflowTask is different - it doesn't have AgentConfig
// We'll create a simplified version for now
export function WorkflowNodeComponent({ data }: { data: WorkflowTask }) {
  // Convert WorkflowTask to AgentConfig-like structure
