...
Audio I/O¶
Out of the box, Strands provides BidiAudioIO to help connect your microphone and speakers to the bidi-agent using PyAudio.
...
The agent emits a BidiInterruptionEvent
BidiAudioIO's internal output buffer is cleared to stop playback
The agent begins responding immediately to the new user input
Text I/O¶
Strands also provides BidiTextIO for terminal-based text input and output using prompt-toolkit.
