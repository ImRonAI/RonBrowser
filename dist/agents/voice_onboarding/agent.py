...
async def main() -> None:
    ...
    model = BidiGeminiLiveModel(
        ...
        provider_config={
            ...
            "inference": {
                # Audio only; let model speak, transcription comes via outputAudioTranscription.
    ...
    try:
        ...
        await asyncio.wait_for(monitor.connected.wait(), timeout=40)
        _emit_state("listening")
        ...
        for idx, question in enumerate(INTERVIEW_QUESTIONS):
            ...
            # Pause mic while we speak the question
            gate.mark_assistant_busy()

            await _ask_question(agent, question, idx)

            # Best-effort: wait for the question to finish speaking.
            try:
                await asyncio.wait_for(monitor.wait_for_response_complete(), timeout=90)
        ...
        # Keep process alive; let Electron restart if needed
    except Exception as exc:  # pragma: no cover
        _emit_state("error")
        print(f"Voice onboarding: fatal error: {exc}", file=sys.stderr)
        raise SystemExit(1)
